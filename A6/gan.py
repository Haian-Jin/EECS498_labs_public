...
from torch import batch_norm, device, nn, optim
...
def discriminator_loss(logits_real:torch.Tensor, logits_fake:torch.Tensor):
  """
  ...
  true_labels = torch.ones_like(logits_real, device=device)
  false_labels = torch.zeros_like(logits_fake, device=device)
  loss_real = F.binary_cross_entropy_with_logits(input=logits_real, target=true_labels, reduction='mean')
  loss_fake = F.binary_cross_entropy_with_logits(input=logits_fake, target=false_labels, reduction='mean')
...
def generator_loss(logits_fake):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  loss = F.binary_cross_entropy_with_logits(input=logits_fake, target=torch.ones_like(logits_fake, device=device))
  ##############################################################################
  #                              END OF YOUR CODE                              #
  ##############################################################################
  return loss
...
def build_dc_classifier():
  ...
  model = nn.Sequential(
                        ...
                        nn.Conv2d(in_channels=1, out_channels=32, stride=1, kernel_size=5), # 24 * 24
                        nn.LeakyReLU(negative_slope=0.01),
                        nn.MaxPool2d(kernel_size=2, stride=2), # 12 * 12
                        nn.Conv2d(in_channels=32, out_channels=64, kernel_size=5, stride=1), # 8 * 8
                        nn.LeakyReLU(negative_slope=0.01),
                        nn.MaxPool2d(kernel_size=2, stride=2), # 4 * 4
                        nn.Flatten(start_dim=1), # 4 * 4 * 64
                        nn.Linear(in_features=1024, out_features=4*4*64),
                        nn.LeakyReLU(negative_slope=0.01),
...
def build_dc_generator(noise_dim=NOISE_DIM):
  ...
  model = nn.Sequential(
                        ...
                        nn.ConvTranspose2d(in_channels=128, kernel_size=4, out_channels=64, stride=2, padding=1),
                        nn.ReLU(),
                        nn.BatchNorm2d(num_features=64),
                        nn.ConvTranspose2d(in_channels=64, kernel_size=4, out_channels=1, stride=2, padding=1),
                        nn.Tanh(),
