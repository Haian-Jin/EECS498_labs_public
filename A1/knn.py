...
def compute_distances_two_loops(x_train, x_test):
  ...
  for i in range(num_train):
        for j in range(num_test):
              dists[i, j] = torch.sqrt(((x_train[i] - x_test[j])**2).sum())
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_no_loops(x_train, x_test):
  ...
  # x_train = torch.flatten(x_train, start_dim=1).unsqueeze(dim=1)
  # x_test = torch.flatten(x_test, start_dim=1).unsqueeze(dim=0)
  # dists = (x_train - x_test).pow(2).sum(dim=2).sqrt()
...
def predict_labels(dists, y_train, k=1):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  _, k_idxs = torch.topk(dists, k=k, dim=0, largest=False)
  for i in range(num_test):
        k_idxs[:, i] = y_train[k_idxs[:, i]]
  y_pred, _ = torch.mode(k_idxs, dim=0)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return y_pred
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  # x_train_folds = [x_train[i::num_folds,:] for i in range(num_folds)]
  # y_train_folds = [y_train[i::num_folds,:] for i in range(num_folds)]
  x_train_folds = []
  y_train_folds = []
  ...
  for k in k_choices:
        ...
        for i in range(num_folds):
              x_train_data = torch.cat([x_train_folds[j] for j in range(num_folds) if j!=i], dim=0)
              y_train_data = torch.cat([y_train_folds[j] for j in range(num_folds) if j!=i], dim=0)
              x_test_data = x_train_folds[i]
              y_test_data = y_train_folds[i]
              classifier = KnnClassifier(x_train_data, y_train_data)
              k_to_accuracies[k].append(classifier.check_accuracy(x_test_data, y_test_data, k=k))
  ##############################################################################
  #                            END OF YOUR CODE                                #
  ##############################################################################
...
def knn_get_best_k(k_to_accuracies):
  ...
  for k in k_to_accuracies.keys():
        mean = torch.tensor(k_to_accuracies[k]).mean().item()
        if (mean > best_mean):
              best_mean = mean
              best_k = k
  ##############################################################################
