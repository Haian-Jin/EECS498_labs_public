...
def multiples_of_ten(start, stop):
  ...
  for i in range(begin, end+1):
    resL.append(10 * i)
  x = torch.tensor(resL, dtype=torch.float64)
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return x
...
def make_one_hot(x):
  ...
  # Replace "pass" statement with your code
  x = torch.tensor(x)
  y = torch.zeros((x.shape[0], x.max()+1), dtype=torch.float32)
...
def zero_row_min(x):
  ...
  # Replace "pass" statement with your code
  y = x.clone()
  min_values, min_idxs = x.min(dim=1)
...
def batched_matrix_multiply(x, y, use_loop=True):
  ...
  if use_loop:
        ...
        z = torch.stack(final_results, dim=0)
  else:
        z = x.bmm(y)
  #############################################################################
  #                            END OF YOUR CODE                               #
