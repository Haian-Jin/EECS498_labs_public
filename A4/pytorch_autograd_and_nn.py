...
class ThreeLayerConvNet(nn.Module):
  ...
  #   1. Convolutional layer with channel_1 5x5 filters with zero-padding of 2  
    ...
    ############################################################################
    # Replace "pass" statement with your code
    self.conv2d1 = nn.Conv2d(in_channels=in_channel, out_channels=channel_1, kernel_size=5, padding=2)
    self.conv2d2 = nn.Conv2d(in_channels=channel_1, out_channels=channel_2, kernel_size=3, padding=1)
    ...
    nn.init.zeros_(self.conv2d1.bias)
    nn.init.zeros_(self.conv2d2.bias)
  ...
  def __init__(self, Cin, Cout, downsample=False):
    ...
    if downsample:
      ...
      self.net = nn.Sequential(
        nn.BatchNorm2d(Cin),
        nn.ReLU(),
        nn.Conv2d(in_channels=Cin, out_channels=Cout, kernel_size=3, padding=1),
        nn.BatchNorm2d(Cout),
        nn.ReLU(),
        nn.Conv2d(in_channels=Cout, out_channels=Cout, kernel_size=3, padding=1),
      )
...
class ResidualBlock(nn.Module):
  def __init__(self, Cin, Cout, downsample=False):
    ...
    if Cout!=Cin and not downsample:
      self.shortcut=nn.Sequential(
        nn.Conv2d(in_channels=Cin, out_channels=Cout, kernel_size=1)
      )
    if downsample:
      self.shortcut=nn.Sequential(
        nn.Conv2d(in_channels=Cin, out_channels=Cout, kernel_size=1, stride=2)
      )
    ############################################################################
    #                                 END OF YOUR CODE                         #
    ############################################################################
...
class ResNet(nn.Module):
  def __init__(self, stage_args, Cin=3, block=ResidualBlock, num_classes=10):
    ...
    ############################################################################
    # Replace "pass" statement with your code
    self.cnn = [ResNetStem(Cin=Cin, Cout=stage_args[0][0])]
    ############################################################################
    #                                 END OF YOUR CODE                         #
    ############################################################################
    for arg in stage_args:
          self.cnn.append(ResNetStage(*arg, block=block))
  ...
  def forward(self, x):
    ...
    # 个人感觉这里是把一一张图片融城一个像素点了
    pool = nn.AvgPool2d((cnn_result.shape[2], cnn_result.shape[3]))
    out = pool(cnn_result)
    # print(out.shape)
...
class ResidualBottleneckBlock(nn.Module):
  def __init__(self, Cin, Cout, downsample=False):
    ...
    self.block = nn.Sequential(
      nn.BatchNorm2d(Cin),
      nn.ReLU(),
      nn.Conv2d(in_channels=Cin, out_channels=Cout//4, kernel_size=1, stride= 2 if downsample else 1),
      nn.BatchNorm2d(Cout//4),
      nn.ReLU(),
      nn.Conv2d(in_channels=Cout//4, out_channels=Cout//4, kernel_size=3, stride=1, padding=1),
      nn.BatchNorm2d(Cout//4),
      nn.ReLU(),
      nn.Conv2d(in_channels=Cout//4, out_channels=Cout, kernel_size=1, stride=1)
    ...
    if downsample:
        ...
        self.shortcut = self.shortcut = nn.Conv2d(in_channels=Cin, kernel_size=1, out_channels=Cout, stride=1)
    ############################################################################
    #                                 END OF YOUR CODE                         #
    ############################################################################
