...
def GenerateGrid(batch_size, w_amap=7, h_amap=7, dtype=torch.float32, device='cuda'):
  ...
  grid = torch.stack([w_grid_idx, h_grid_idx], dim=-1) # concatenate the two tensor in the new dim
...
def ReferenceOnActivatedAnchors(anchors, bboxes, grid, iou_mat, pos_thresh=0.7, neg_thresh=0.3, method='FasterRCNN'):
  ...
  # activated/positive anchors, find the index of bbox that has the maximum IoU with the anchor
  max_iou_per_anc, max_iou_per_anc_ind = iou_mat.max(dim=-1) # iou_mat has a shape of Bx(A*H*W)xN
  if method == 'FasterRCNN':
    # iou_mat has a shape of Bx(A*H*W)xN
    max_iou_per_box = iou_mat.max(dim=1, keepdim=True)[0] # [0] means choosing values instead of indexes, result shape is (B, 1, N)
    # shape of mask is (B, A*H*W, N)
    ...
    # "max_iou_per_anc_ind" has a shape of (B, (A*H*W))
    # for torch.gather(), the output shape is the same as the index shape
    ...
    # grid is (B, H, W, 2)
    mah_dist = torch.abs(grid.view(B, -1, 2).unsqueeze(2) - bbox_centers.unsqueeze(1)).sum(dim=-1) # Bx(H*W)xN
    min_mah_dist = mah_dist.min(dim=1, keepdim=True)[0] # Bx1xN, find which grid the bounding box is located
    # find the location of grid that is closed to one GT_box  
    grid_mask = (mah_dist == min_mah_dist).unsqueeze(1) # Bx1x(H'xW')xN

    reshaped_iou_mat = iou_mat.view(B, A, -1, N) # (B, A, H*W, N)
    # find the anchor that has the maximum IoU score. Each grid location will find one anchor
    ...
    # grid_mask will locate (H*W) and anc_mask will locate anchor A
    activated_anc_mask = (grid_mask & anc_mask).view(B, -1, N) # (B, A*H*W, N)
    activated_anc_mask &= bbox_mask.unsqueeze(1) # get rid of those invalid bboxes
    
    # one anchor could match multiple GT boxes
    activated_anc_ind = torch.nonzero(activated_anc_mask.view(-1)).squeeze(-1) # return of torch.nonzero will be (numberX, 1), so needs to squeeze
    ...
    bboxes = bboxes[:, :4]
    activated_anc_ind = (activated_anc_ind / float(activated_anc_mask.shape[-1])).long() # divided by N, so that the index is in the range of (0, A*H*W)
  ...
  negative_anc_ind = negative_anc_ind[torch.randint(0, negative_anc_ind.shape[0], (activated_anc_ind.shape[0],))] # make sure the number of negative anchors is the same as the number of positive anchors
...
def ObjectClassification(class_prob, GT_class, batch_size, anc_per_img, activated_anc_ind):
  ...
  Inputs:
  - class_prob: Predicted class logits, (M, C)
